---
title: "Preparing toxEval Analysis"
...
The bottom section "R Code:" shows the exact code you could use to run directly in R to reproduce the analysis. The code can be copied and pasted directly into the R console, or (preferred for reproducibility!) saved in an R script. One important note is that the local path to the file is not recorded, so if your data file is not located in your current working directory, you'll need to add the path to your file name.
