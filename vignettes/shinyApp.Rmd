...
All of the output on the Shiny app can be created directly in R as well. Using the app is a quick and convenient way to explore your data. For more customization, the R-code to produce each graph and table is displayed in the app which serves as a good starting-point for a custom analysis. The help file each function provides instructions and example workflows for their use.
...
The top main section includes a variety of tabs to choose from to explore the data in different ways. Each section is described in the section "Main Output".

The bottom section, "R Code:", displays the R code to reproduce the analysis directly in R using the `toxEval` R-package functions. The code can be copied and pasted directly into the R console, or (preferred for reproduciblity!) saved in an R script. One important note is that the local path to the file is not recorded, so if the data file is not located in the current working directory, a file path will need to be added to the file name.
...
The table to the right shows the main function that each tab in the app uses:
