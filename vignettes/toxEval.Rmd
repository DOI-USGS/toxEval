...
for(i in names(passiveData)[siteColumnsIndex]){
  ...
  maxRatioBySite[maxRatioBySite$site == i,"chemical"] <- maxRatio
  maxRatioBySite[maxRatioBySite$site == i,"endpoint"] <- maxRatio
}
...
So for the passive dataset, the maximum ratio of the measured data to the endpoints for all chemicals at all sites is `r max(maxRatioBySite$ratio)`. This is site: `r maxRatioBySite$site[maxRatioBySite$ratio == max(maxRatioBySite$ratio)]`.
