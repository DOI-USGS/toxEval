...
The `toxEval` R-package includes a set of functions to analyze, visualize, and organize measured concentration data as it relates to ToxCast data (default) or other user-selected chemical-biological interaction benchmark data such as water quality criteria. These functions allow great flexibly for exploring the potential biological affects of measured chemicals. Also included in the package is a browser-based application made from the `shiny` R-package. The application is based on functions within the R-package and includes many convenient analysis and visualization options for users to choose. Use of the functions within the R-package directly allows for additional flexibility within the functions beyond what the application offers, and options for the user to interact directly with the data.  

This vignette provides a general overview of the concepts within `toxEval`, definitions of common terminology used throughout the package and application, and links to information to help understand fundamentals of the ToxCast database used within `toxEval`. In addition, instructions are provided for installing and updating the package from the Geological Survey R Archive Network (GRAN). 
...
In order to get consistent EAR results, it is important to use the correct units. The `toxEval` package assumes all measured concentrations are repored in micrograms per liter (μg/L). ToxCast data is reported in log(μM), so the `toxEval` package automatically performs the unit conversion.
...
Following these instructions will add the USGS R repository (GRAN) the users R profile. For users who have previously installed R and RStudio, GRAn can be added the R profile by pasting the following into your R console:
...
This section assumes GRAN was added to your R profile as described above.
