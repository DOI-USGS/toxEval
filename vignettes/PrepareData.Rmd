...
The "Data" sheet needs to have at a minimum of 4 columns: "CAS", "SiteID", "Value", and "Sample Date". They can be in any order, but the first row of the sheet should be the header (column names).
...
The "Chemicals" sheet needs to have at a minimum of 2 columns: "CAS" and "Class". They can be in any order, but the first row of the sheet should be the header (column names).
...
The "Sites" sheet needs to have a minimum of 4 columns: "SiteID", "Short Name", "dec_lon", and "dec_lat". They can be in any order, but the first row of the sheet should be the header (column names).
...
The "Exclude" sheet is optional. If used, the "Exclude" sheet needs to have at a minimum 2 columns: "CAS" and "endPoint". They can be in any order, but the first row of the sheet should be the header (column names).

* endPoint: A character column to define a specific ToxCast endpoint to ignore. IF the "CAS" in the corresponding row is empty, the endPoint will be completely excluded from the `toxEval` analysis. For example, if it was decided that TOX21_p53_BLA_p3_ratio was not an appropriate endPoint to consider for a given analysis, it can be excluded by adding it to the Exclude sheet in the endPoint column. If the "CAS" in the corresponding row is NOT empty, the specific chemical/endPoint combination will be excluded from the analysis. 

* CAS: A character column defining the chemicals via their Chemical Abstracts Service (CAS) registry. IF the "endPoint" in the corresponding row is empty, the chemical will be completely excluded from the `toxEval` analysis. For example, maybe it was discovered that DEET was contaminated in the sampling method. We can exclude all DEET measurements by adding 134-62-3 to the Exclude sheet. IF the "endPoint" in the corresponding row is NOT empty, the specific chemical/endPoint combination will be excluded from the analysis.
...
For the functions to work seamlessly with all `toxEval` functions, the "Benchmarks" sheet needs to have a minimum of 5 columns: "CAS", "Chemical", "endPoint", "Value", and "groupCol". They can be in any order, but the first row of sheet should be the header (column names).
