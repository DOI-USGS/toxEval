...
## Chemical Summary Tables: Water Samples (ToxCast)
...
### ToxCast Individual EndPoints: Water Samples
...
## Site Summaries: Water Samples (ToxCast)
...
sToxWS <- ggplot(summary_display2, aes(x=site, y=EAR)) +
  ...
  annotation_custom(grob=textGrob("nEndPoints:", gp = gpar(fontface = "italic", fontsize = 8)),xmin = -0.9, xmax = -0.9, ymin = -1.5, ymax = -1.5)
...
chemToxWS <- ggplot(chemicalSummary_display, aes(x=chnm, y=EAR)) +
  ...
  annotation_custom(grob=textGrob("nEndPoints:", gp = gpar(fontface = "italic", fontsize = 8)),xmin = -0.7, xmax = -0.7, ymin = -1.5, ymax = -1.5)
...
## Passive Detection Levels

```{r warning=FALSE, echo=FALSE}

detLevels_passive  <- select(passiveData, Chemical, MQL, CAS) %>%
  right_join(endPoint_passive, by=c("CAS"="casn")) %>%
  select(-mlWt, -conversion,   -Chemical, -CAS) %>%
  melt(id.vars = c( "MQL","chnm", "Units")) %>% 
  filter(!is.na(value)) %>%
  mutate(variable=as.character(variable)) %>%
  rename(endPointValue=value, endPoint=variable) %>%
  mutate(EAR=MQL/endPointValue) %>%
  filter(EAR >= 0.1) %>%
  select(Chemical=chnm, EAR, endPoint, MQL, Units) %>%
  arrange(desc(EAR)) 


datatable(detLevels_passive, rownames = FALSE,options = list(pageLength = 25))  %>% 
    formatRound(c("EAR","MQL"), digits = 1)

```


## Chemical Summary Tables: Passive Data

```{r, echo=FALSE, results='asis'}
chemSum2_passive_dt <- chemSum2_passive[,-7] %>%
  filter(freq > 0)
...
### Individual EndPoints: Passive Data
Looking at individual endpoints (filtering only rows that have some hit):
...
sp <- ggplot(summary_display2_passive, aes(x=site, y=EAR)) +
  ...
  geom_text(data=data.frame(), aes(x=siteLimits_passive$site,
                                   y=-.75, label=siteLimits_passive$nEndPoints),size=3) +
  annotation_custom(grob=textGrob("nEndPoints:", gp = gpar(fontface = "italic", fontsize = 8)),xmin = -0.7, xmax = -0.7, ymin = -0.75, ymax = -0.75)
...
cp <- ggplot(chemicalSummary_display_passive, aes(x=chnm, y=EAR)) +
  ...
  labs(x = "Chemical Name (ordered by fraction of sites with hits)")+
  geom_text(data=data.frame(), aes(x=chemLimits_passive$chnm, y=-0.75,
                                   label=chemLimits_passive$nEndPoints),size=3) +
  annotation_custom(grob=textGrob("nEndPoints:", gp = gpar(fontface = "italic", fontsize = 8)),xmin = 0.5, xmax = 0.5, ymin = -0.75, ymax = -0.75)
...
## Water Quality Guidelines: Detection Levels
...
detLevels_BM  <- data.frame(dl_pcode=names(detectionLimits),
  ...
  mutate(EAR=minDetLevel/endPointValue) %>%
  filter(EAR >= 0.1) %>%
  select(Chemical=chnm, Class=class, EAR, endPoint, minDetLevel, Units) %>%
  arrange(desc(EAR)) 


datatable(detLevels_BM, rownames = FALSE,options = list(pageLength = 25))  %>% 
    formatRound(c("EAR","minDetLevel"), digits = 1)

```


## Chemical Summary Tables: Water Samples (Water Quality Benchmarks)
...
## Site Summaries: Water Samples (Water Quality Benchmarks)
...
## Water Quality Benchmarks: Water Samples Sites Boxplots
...
ggplot(summary_display2_BM, aes(x=site, y=EAR)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust=0.25), 
        legend.position = "none")+
  ylim(-2,50) + 
  ...
  labs(x = "Site (ordered by number of chemicals with hits)") +
  geom_text(data=data.frame(), aes(x=siteLimits_BM$site, y=-1.5,
                                   label=siteLimits_BM$nEndPoints),size=3) 
```

## Water Quality Benchmarks: Water Samples Chemical Boxplots
...
ggplot(chemicalSummary_display_BM, aes(x=chnm, y=EAR)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust=0.25), 
        legend.position = "none") +
  ylim(-2,50) + 
  ...
  labs(x = "Chemical Name (ordered by fraction of sites with hits)")+
  geom_text(data=data.frame(), aes(x=chemLimits_BM$chnm, y=-1.5,
